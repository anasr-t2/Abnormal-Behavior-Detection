		...
		if (mode)
		{
			...
			for (CvTracks::const_iterator it = tracks.begin(); it != tracks.end(); ++it)
			{
				...
				if (!it->second->inactive)
				{
					...
					if (haa != NULL)
						rr = haa;
					//cvPutText(imgDest, rr, cvPoint((int)it->second->centroid.x, (int)it->second->centroid.y), font, CV_RGB(255., 0., 0.));

					if (wrongDir == 1){
						cvPutText(imgDest, "abnormality: LR", cvPoint(0, 20), font, CV_RGB(255., 0., 0.));
					}
					else if (wrongDir == 0){
						cvPutText(imgDest, "abnormality: RL", cvPoint(0, 20), font, CV_RGB(255., 0., 0.));
				...
				if (it->second->inactive){
					...
					//(x2 - x1)*(y1-y2)
					//area = ((it->second->maxx - 1) - (it->second->minx))*((it->second->miny) - (it->second->maxy - 1));
					int l1 = sqrt(((it->second->maxy - 1) - (it->second->miny))*((it->second->maxy - 1) - (it->second->miny)));
					int l2 = sqrt(((it->second->maxx - 1) - (it->second->minx))*((it->second->maxx - 1) - (it->second->minx)));
					area = l1*l2;
					//cout << it->second->maxx - 1 << "  " << it->second->minx << "   " << it->second->miny << "   " << it->second->maxy - 1 << endl;
