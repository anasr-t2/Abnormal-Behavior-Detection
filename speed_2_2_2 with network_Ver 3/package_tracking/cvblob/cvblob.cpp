...
namespace cvb
{
	...
	{
		featureClass *obj = new featureClass;
		//featureClass obj;
		...
		if (mode)
		{
			...
			avg.size = sum / counter;

			/*cout << "Avg Size: " << avg.size << "  ";
			cout << "blob Size:  " << blob->area << endl;*/


			//obj->size = blob->area;

			_Destroy(&obj);
			//cout << "tariq qawy gdn" <<obj->ID<< "   " <<avg.ID << endl;
			...
			//hna b y rsm rectangle a7mar (red rectangle) 3la el blob
			//la7z el points bta3t el rectangl lo 3ayz tsa3'r aw tkbr el rectangle 3la el motion object 
			if (mode&CV_BLOB_RENDER_BOUNDING_BOX){
				if (blob->area > avg.size * 2 /*|| blob->area < avg.size * 0.5*/)
